  ...
  try {
    ...
    await query(
      `INSERT INTO gradebook (
        ...
        assessment_type, title, marks, max_marks, grade, date, notes
      ...
      [
        ...
        calculatedGrade, date || new Date(), notes
      ]
    );
    ...
    for (const gradeEntry of grades) {
      try {
        ...
        await query(
          `INSERT INTO gradebook (
            id, class_id, student_id, subject_id, teacher_id,
            assessment_type, title, marks, max_marks, grade, date
  ...
  try {
    ...
    await query(
      `UPDATE gradebook SET
        ...
        updated_at = NOW()
       WHERE id = $5`,
      [marks, maxMarks, calculatedGrade, notes, req.params.id]
    );
...
router.delete('/:id', authenticate, requireRole(['admin', 'teacher']), async (req, res) => {
  try {
    await query('DELETE FROM gradebook WHERE id = $1', [req.params.id]);
