...
async function sendSMSViaProvider(phone, message) {
  ...
  const senderId  = process.env.AT_SENDER_ID || 'SkulMgr';

  if (!apiKey) {
    logger.warn('Africa\'s Talking API key not configured — SMS logged only');
  ...
  try {
    ...
    return { success: false, error: recipient?.status || 'Unknown failure' };
  } catch (err) {
...
// ─── POST /sms/send — send to one recipient ───────────────────────────────────
router.post('/send', async (req, res) => {
  try {
    ...
    // Auto-resolve phone if recipient_id provided
    if (!phone && recipient_id) {
      ...
      if (recipient_type === 'parent') {
        ...
        rows = await query('SELECT phone FROM users WHERE id=$1 AND tenant_id=$2', [recipient_id, tid]);
      } else if (recipient_type === 'student') {
        // Send to parent of student
        rows = await query(
...
// ─── POST /sms/bulk — send to multiple recipients ────────────────────────────
router.post('/bulk', async (req, res) => {
  try {
    ...
    if (target === 'all_parents') {
      ...
      const rows = await query(
        'SELECT id, phone FROM users WHERE role=\'teacher\' AND tenant_id=$1 AND phone IS NOT NULL AND phone != \'\'',
        [tid]
      );
    ...
    res.json({ success: true, data: { sent, failed, total: recipients.length } });
  } catch (err) {
...
router.get('/logs', async (req, res) => {
  try {
    ...
    const countRows = await query(
      'SELECT COUNT(*) FROM sms_messages WHERE tenant_id=$1',
      [tid]
    );
...
// ─── GET /sms/templates — quick message templates ────────────────────────────
router.get('/templates', async (req, res) => {
  const templates = [
    ...
    { id: 5, category: 'transport',     label: 'Transport Alert',      text: 'TRANSPORT: {student_name} was not picked up this morning on route {route_name}. Contact driver: {driver_phone}. — {school_name}' },
