...
export function TenantsPage() {
  ...
  const openCreate = () => {
    ...
    setForm({ school_name: '', email: '', phone: '', address: '', city: '', county: '', admin_email: '' });
    setFormError('');
    setModal('create');
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    setFormError('');
    if (!form.school_name || !form.email) { setFormError('School name and email are required.'); return; }
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="flex flex-wrap gap-x-4 gap-y-1 mt-1">
                        <span className="text-sm text-gray-600 flex items-center">
                          <Mail className="h-3 w-3 mr-1 text-gray-400" />{t.admin_email || t.email}
                        </span>
                        {t.phone && <span className="text-sm text-gray-600 flex items-center">
                          <Phone className="h-3 w-3 mr-1 text-gray-400" />{t.phone}
                        </span>}
                      ...
                      {getExpiryDate(t) && (
                        <p className="text-xs text-gray-400 mt-1 flex items-center">
                          ...
                          {getStartDate(t) ? `${new Date(getStartDate(t)!).toLocaleDateString()} → ` : ''}
                          Expires: {new Date(getExpiryDate(t)!).toLocaleDateString()}
                        </p>
                      )}
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              ...
              {[
                ...
                { name: 'email', label: 'School Email *', placeholder: 'info@school.ac.ke', type: 'email' },
                { name: 'admin_email', label: 'Admin Login Email', placeholder: 'admin@school.ac.ke', type: 'email' },
                { name: 'phone', label: 'Phone', placeholder: '07XXXXXXXX' },
                { name: 'address', label: 'Address', placeholder: 'P.O. Box, Street' },
                { name: 'city', label: 'City / Town', placeholder: 'Nairobi' },
              ].map(f => (
                <div key={f.name}>
              ...
              <div>
                ...
                <select name="county" value={form.county} onChange={e => setForm(p => ({ ...p, county: e.target.value }))}
                  ...
                  {['Nairobi', 'Mombasa', 'Kisumu', 'Nakuru', 'Eldoret', 'Kiambu', 'Machakos', 'Meru', 'Nyeri', 'Kakamega', 'Other'].map(x => <option key={x}>{x}</option>)}
                </select>
              </div>
                  ...
                  {payments.map((p: any, i: number) => (
                    <div key={i} className="flex items-center justify-between p-3 border rounded-lg">
                      <div>
                        ...
                        <p className="text-xs text-gray-500">{p.payment_method || 'M-Pesa'} · {p.mpesa_receipt_number || p.payment_number || '—'}</p>
                        <p className="text-xs text-gray-400">{new Date(p.transaction_date || p.created_at).toLocaleString()}</p>
                      </div>
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm p-6">
            ...
            <div className="bg-blue-50 rounded-lg p-3 mb-5">
              <p className="text-sm text-blue-700">
                New expiry: <strong>{(() => {
                  const base = selectedTenant.subscription_ends_at
                    ? new Date(selectedTenant.subscription_ends_at)
                    : new Date();
